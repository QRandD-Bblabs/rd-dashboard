...
import { roadmapExploreNote } from '../data/roadmap';
import { useRoadmapRemote } from '../data/remote/useRoadmapRemote';
import { TBDTag } from '../components/TBDTag';

export function Roadmap() {
  const { now, next, later, explore, retailPhases, loading } = useRoadmapRemote();

  const columns = [
    { title: 'NOW', subtitle: 'Active, consuming capacity', items: now, accent: 'border-t-[#F2545B]' },
    { title: 'NEXT', subtitle: 'Shaping / Discovery', items: next, accent: 'border-t-[#91BFE5]' },
    { title: 'LATER', subtitle: 'Sequenced, not scheduled', items: later, accent: 'border-t-[#B9C9E9]' },
    { title: 'EXPLORE', subtitle: 'Research topics, not initiatives', items: explore, accent: 'border-t-[#0B0A07]/20' },
  ];
        ...
        {columns.map((col) => (
          <Card key={col.title} className={`border-t-4 ${col.accent}`}>
            ...
            <div className="space-y-2">
              ...
              {col.items.length === 0 && (
                <div className="rounded-xl border border-dashed border-[#0B0A07]/15 px-3 py-6 text-center text-xs text-[#0B0A07]/35">Empty</div>
              )}
            </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {retailPhases.map((phase, i) => (
            <div key={phase.id} className="flex items-center gap-2">
              ...
              {i < retailPhases.length - 1 && <span className="text-[#0B0A07]/25">→</span>}
            </div>
          ))}
        </div>
